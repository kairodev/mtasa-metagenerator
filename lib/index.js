...
// Relative Path
const relativePath = (actualDir, file) => {
    const fullPath = path.relative(actualDir, file);
    return fullPath.replace(/\\/g, '/');
};

// Write XML
const writeMeta = (theFiles, currentDir) => {
    let sharedScripts = '';
    let otherScripts = '';
    let files = '';

    theFiles.forEach((file) => {
        const thePath = relativePath(currentDir, file);
...
// Função principal que gera o arquivo meta.xml
...
// Exportando as funções para serem usadas externamente
module.exports = {
    deleteOldMeta,
    listFiles,
    relativePath,
